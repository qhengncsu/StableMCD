...
data2 = data.frame(h=seq(0.5,0.975,by=0.025),mean_q2 = bootstrap_result1$iim,sd_q2 = bootstrap_result1$wd_sds,
                   mean_q3 = bootstrap_result2$iim,sd_q3 = bootstrap_result2$wd_sds,
                   mean_q10 = bootstrap_result3$iim,sd_q10 = bootstrap_result3$wd_sds)
...
plot2  = ggplot(data2,aes(x=h))+
  ...
  scale_color_manual("Number of PCs",breaks=c("q=2","q=3","q=10"),
                     values=c("navyblue","darkred","darkgreen"))+
  labs(y = "IIM", x = "h/n", title="IIM on Glass Data")+theme_bw()+
  theme(plot.title = element_text(hjust = 0.5), legend.position = "None", text = element_text(size=12))
