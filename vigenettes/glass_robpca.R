...
data1 = data.frame(h=seq(0.5,0.975,by=0.025),mean_q2 = bootstrap_result1$insta_means,sd_q2 = bootstrap_result1$insta_sds,
                   mean_q3 = bootstrap_result2$insta_means,sd_q3 = bootstrap_result2$insta_sds,
                   mean_q10 = bootstrap_result3$insta_means,sd_q10 = bootstrap_result3$insta_sds)
...
data2 = data.frame(h=seq(0.5,0.975,by=0.025),mean_q2 = bootstrap_result1$final_score,sd_q2 = bootstrap_result1$wd_sds,
                   mean_q3 = bootstrap_result2$final_score,sd_q3 = bootstrap_result2$wd_sds,
                   mean_q10 = bootstrap_result3$final_score,sd_q10 = bootstrap_result3$wd_sds)
...
plot2  = ggplot(data2,aes(x=h))+
  ...
  scale_color_manual("Number of PCs",breaks=c("q=2","q=3","q=10"),
                     values=c("navyblue","darkred","darkgreen"))+
  labs(y = "Final Score", x = "h/n", title="Final Score on Glass Data")+theme_bw()+
  theme(plot.title = element_text(hjust = 0.5), legend.position = "None", text = element_text(size=12))


robpca3 = PcaHubert(glass,k=3,alpha=0.5)
SDs = robpca3$sd
ODs = robpca3$od
...
plot3 = ggplot()+ geom_point(data=data3, aes(x=X1,y=X2),size=3, color='blue',shape=1) + 
  ...
  theme(plot.title = element_text(hjust = 0.5), legend.position = "none", text = element_text(size=12)) +
  geom_vline(data=data3, aes(xintercept=robpca2$cutoff.sd),color='purple')+
  geom_hline(data=data3, aes(yintercept=robpca2$cutoff.od),color='purple')+
