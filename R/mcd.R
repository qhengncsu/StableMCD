...
mahalanobis <- function(x, center, cov, inverted=F, ...){
  x <- if (is.vector(x)) 
...
bootstrap_mcd <- function(x, hs, MD = T, B=50){
  n <- nrow(x)
  instabilities = list()
  for(i in 1:length(hs)){
    instabilities[[i]] = rep(0,B)
  }
  depth = proj_depth(x,x,1,multiplier=1000)
  for(b in 1:B){
    ...
    depths1 = depth[index1_bootstrap]
    depths2 = depth[index2_bootstrap]
    ...
    for(i in 1:length(hs)){
      ...
      if(MD){
        ...
        depths1 = proj_depth(x,x1[index1,],1)
        depths2 = proj_depth(x,x2[index2,],1)
  ...
  for(i in 1:length(hs)){
    ...
    trimeans[i] = mean(instabilities[[i]])
    h = hs[i]
    retain = 1 - (1 - pbinom(h,n,h/n))^2
    trimeans[i] = mean(instabilities[[i]][order[1:(retain*B)]])
    sds[i] = sd(instabilities[[i]])
  }
  best_index = which(trimeans == min(trimeans))
  best_h = hs[best_index]
  return(list(best_h=best_h,trimeans=trimeans,sds=sds,instabilities=instabilities))
}
